...
def get_anime_newseason(page = 0):
    ...
    return res_newseason_list or {"status":"204", "reason":"Wrong Page Id"}
...
def get_anime_recent(type = 1, page = 0):
    ...
    return res_recent_list or {"status":"204", "reason":"Wrong Page Id"}
...
def get_anime_details(id):  
    ...
    genres = [x.get('title') for x in genre]
    ...
    res_detail_list = [
        {
            "title": f"{title}",
            "release": f"{release}",
            "other_names": f"{other_names}",
            "type": f"{type}",
            "status": f"{status}",
            "genre": f"{genres}",
            "total_episode": f"{totalep}",
            "image": f"{img}",
            "description": f"{summary}",
        }
    ]
    return res_detail_list or {"status":"204", "reason":"Wrong Anime Id"}
...
def get_anime_episode(email, password, id, episode):
    ...
    if not auth_gogo:
        return "invalid e-mail and password"
    res_episode_list = []
    ...
    return res_episode_list or {
        "status": "204",
        "reason": "Wrong Anime Id And Episode",
    }
